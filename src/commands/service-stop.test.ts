...
import { serviceStop } from "./service-stop.js";
...
describe("serviceStop", () => {
  ...
  it("should successfully stop a service", async () => {
    ...
    vi.mocked(shellModule.executeShell).mockResolvedValue({
      stdout: "Container web-service Stopped",
      stderr: "",
      exitCode: 0,
    });
    ...
    expect(shellModule.executeShell).toHaveBeenCalledWith(
      "docker",
      ["compose", "-f", "docker-compose.yml", "stop", "web-service"],
      {
        cwd: "/test/dir/web",
        timeout: 60000,
      }
  ...
  it("should handle already stopped service (idempotent)", async () => {
    vi.mocked(configModule.loadConfig).mockResolvedValue({
      config: {
        runner: "claude",
        services: {
    ...
    expect(console.log).toHaveBeenCalledWith(
      expect.stringContaining("is already stopped")
    );
  });
  ...
  it("should throw error when docker command fails", async () => {
    ...
    await expect(
      ...
      'Failed to stop service "web": Error: Cannot connect to the Docker daemon'
    );
  });
