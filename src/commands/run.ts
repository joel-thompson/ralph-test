...
import { validateWorkingDirectory, validateRequiredFiles } from '../utils/validation.js';
...
export async function run(
  ...
  const validation = await validateRequiredFiles(workingDirectory, requiredFiles);
  ...
  for (let iteration = 1; iteration <= maxIterations; iteration++) {
    ...
    try {
      // Call Claude CLI
      const response = await runner.runClaude(promptPath);
      ...
      if (error instanceof Error) {
        throw new CommandError(
          `Failed to run Claude CLI: ${error.message}`
        );
      }
      throw error;
    }
